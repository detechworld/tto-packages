...
sjcl.codec.deip = {
    ...
    keysFromPassword: function(account, password) {
        ...
        for (var i = 0; i < sjcl.codec.deip.ROLES.length; i++) {
            var role = sjcl.codec.deip.ROLES[i];
    ...
    signRecoverably: function(sec, hash, paranoia, fixedKForTesting) {
         ...
         * its X coordinate).
         *
         * note that in the Deip blockchain, all public keys are canonically compressed and therefore the
    ...
    recoverPublicKey: function(hash, sig) {
        ...
        for (var j = 0; j <= 1; j++) {
            ...
            var y = sjcl.codec.deip._yFromX(x, hasOddParity);
    ...
    serializePublicKey: function(key, net) {
        net = net || sjcl.codec.deip.MAINNET;
    ...
    deserializePublicKey: function(pubKey, net) {
        ...
        var payload = sjcl.codec.base58Check.toBits(
            pubKey.slice(net.pubPrefix.length),
            sjcl.codec.deip.keyChecksum
        );
        ...
        var y = sjcl.codec.deip._yFromX(x, isOdd);
    ...
    deserializePrivateKey: function(wif, header) {
        header = header || sjcl.codec.deip.MAINNET.privHeader;
        var curve = sjcl.ecc.curves.k256;
    ...
    _yFromX: function(x, shouldBeOdd) {
        var CURVE = sjcl.ecc.curves.k256;
        var PIDENT = sjcl.codec.deip._getPident();
    ...
    _getPident: function() {
        if (!sjcl.codec.deip.PIDENT) {
            sjcl.codec.deip.PIDENT = sjcl.ecc.curves.k256.field.modulus
                .add(1)
                .normalize()
                .halveM()
                .halveM()
